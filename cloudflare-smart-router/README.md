...
- `CLIENT_KEYS_JSON`：访问 Key 到用户标识的 JSON，例如 `{"用户Key":"user-001"}`。Worker 同时兼容 Cloudflare JSON 对象绑定与 JSON 字符串绑定。
...
运行 `npm install` 后执行 `npm run check`。Worker 代码变更使用 `npx wrangler deploy --keep-vars` 发布；生产环境的用户访问 Key 保存在 Cloudflare 远端变量中，禁止用不带 `--keep-vars` 的普通部署覆盖远端变量。发布后必须确认新版本仍包含 `CLIENT_KEYS_JSON` 绑定。仅新增符合命名规则的供应商路线包或对应 API Key Secret 时，平台用户无需更新版本。
