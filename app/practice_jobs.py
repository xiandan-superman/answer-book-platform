...
_GENERATION_DEADLINE_SECONDS = bounded_env_int(
    "PRACTICE_GENERATION_DEADLINE_SECONDS", 7200, 30, 14400
)
...
def _is_generated_upload_title(value: Any) -> bool:
    """Return whether a name was generated by the paste/upload machinery."""
    title = _clean_task_title(value)
    if not title:
        return False
    stem = Path(title).stem
    return bool(re.match(r"^(?:粘贴截图|截图|剪贴板图片|clipboard|pasted[-_ ]?image|image)[-_ ]?(?:\d|$)", stem, re.I))
...
def _default_task_title(payload: dict[str, Any]) -> str:
    ...
    if explicit and not _is_generated_upload_title(explicit):
        return explicit
    ...
    source_files = [item for item in payload.get("source_files") or [] if isinstance(item, dict)]
    if source_files:
        source_label = "知识材料图片" if str(payload.get("source_mode") or "") == "knowledge" else "图像原题"
        if len(source_files) > 1:
            source_label = f"{len(source_files)}份{source_label}"
        try:
            count = max(0, int(payload.get("count") or 0))
        except (TypeError, ValueError):
            count = 0
        difficulty = _clean_task_title(payload.get("difficulty"))
        details = [f"{count}题" if count else "", difficulty]
        suffix = " · ".join(item for item in details if item)
        return f"{source_label}{f' · {suffix}' if suffix else ''}"
    return "未命名材料"
...
def _batch_task_title(batch_id: str) -> str:
    ...
    for path in sorted(PRACTICE_JOB_DIR.glob("generation_*.json"), reverse=True):
        ...
        if _is_generated_upload_title(title):
            continue
        if title:
            return title
    return ""
...
def list_practice_jobs(limit: int = 100, *, include_history_completed: bool = False) -> list[dict[str, Any]]:
    ...
    for path in sorted(PRACTICE_JOB_DIR.glob("generation_*.json"), reverse=True):
        ...
        payload = record.get("payload") if isinstance(record.get("payload"), dict) else {}
        title = _clean_task_title(record.get("title"))
        if title in {"按题出题", "知识点出题", "范围解析", "蓝图设计", "题目生成"} or _is_generated_upload_title(title):
            record["title"] = _default_task_title(payload)
        record["provider"] = str(record.get("provider") or payload.get("provider") or "")
        record["model"] = str(record.get("model") or payload.get("model") or "")
        record.pop("payload", None)
        record.pop("result", None)
        rows.append(record)
