...
def _practice_document_contract_issues(archive: ZipFile, root, namespaces: dict[str, str]) -> list[str]:
    ...
    if "word/fontTable.xml" not in archive.namelist():
        issues.append("练习 Word 契约：缺少字体声明。")
    else:
...
def _build_practice_question_docx_c(
    data: dict[str, Any],
    *,
...
def _build_practice_solution_docx_c(data: dict[str, Any]) -> bytes:
...
def _build_practice_docx_c(data: dict[str, Any]) -> bytes:
...
def build_practice_question_docx(
    ...
    *,
    ...
    return _build_practice_question_docx_c(data, progress_callback=progress_callback)


def build_practice_solution_docx(data: dict[str, Any]) -> bytes:
    return _build_practice_solution_docx_c(data)


def build_practice_docx(data: dict[str, Any]) -> bytes:
    """Combined export using the sole Pandoc C engine."""
    return _build_practice_docx_c(data)
