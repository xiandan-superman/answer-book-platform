    ...
    *,
    ...
    user_payload = {
        ...
        "hard_rules": [
            ...
            "计算题可返回 calculation_contract；程序不根据题目或答案措辞要求该账本：requested_outputs 只能复述原题明确要求，不得把教材中出现的相关量扩展成题目要求；result_quantities 的 formula_index 必须指向完全相同结果值的 result 公式；同一总体的组成、分数或概率必须在 partitions 中共用同一 basis 并加和为 expected_total。",
            ...
            "若模型选择声明 intermediate_quantities 和 transitions，应满足：父项、新生子项、剩余子项使用同一全局基准，子项之和等于父项；局部分数只能乘它所属的父项。程序仅检查明确声明的引用与同基准数值关系，不从正文推断必填账本。",
