...
def test_program_answer_image_hint_does_not_make_delivery_fail(tmp_path) -> None:
...
def test_main_model_adopted_answer_image_must_be_delivered(tmp_path) -> None:
    source = tmp_path / "source.png"
    source.write_bytes(b"image")
    _write(tmp_path / "structured_exam.json", {"items": [{"question_id": "q1", "stem": "根据下图画出对应曲线", "image_refs": [str(source)], "source_image_required": True}]})
    _write(tmp_path / "answer_fragments.json", {"fragments": [{"question_id": "q1", "generated_images": [{"asset_id": "img_sha256_checked"}], "blocks": [{"label": "原题图", "segments": [{"type": "image_ref", "path": str(source), "role": "source_question_image"}]}]}]})
    summary, issues, _ = figure_delivery_findings(tmp_path)
