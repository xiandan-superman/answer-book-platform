...
更新进度接口只返回版本、阶段、公开提示和字节数，并且只允许回环地址读取。它不要求随服务进程轮换的本机操作令牌，确保重启前已打开的页面仍能读取最终 100% 状态；所有更新触发、配置和其他本机特权操作继续要求令牌。监督器在同一次运行中为更新前后的服务复用同一枚临时令牌，新版健康检查完成后网页自动刷新，以加载新静态资源并建立新的页面会话。新版启动时若持久化进度仍停留在任一解压、备份、依赖或启动阶段，但当前应用版本已达到目标版本，服务必须自动收口为完成，不能继续显示 99%。
...
- 更新 API 只能由运行服务的本机调用。
- 更新进度只记录版本、阶段、字节数和公开错误，不记录 API Key、任务材料或用户输出内容；只读进度允许本机回环页面跨服务重启读取，其他特权接口仍必须验证当前监督器令牌。
