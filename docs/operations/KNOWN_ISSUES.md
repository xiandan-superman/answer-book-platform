...
## 2026-09-10｜供应商持续超时被预算、熔断和教材证据合并缺陷放大

- status: implemented_locally; targeted_and_full_gates_passed; not_deployed; not_released
- affected_flow: 真题解析的知识规划、教材证据选择、答案生成，两类生题的共享供应商请求通道与错误展示。
- observed: 供应商多组流式读取超时后仍反复探测到总预算耗尽；单题在供应商闸门排队时已开始执行计时；教材扩展候选的后续知识点可被全局截断，空扩展结果可覆盖已确认引用。
- attribution: 原始流式停滞属 `provider_transport`；排队计时、固定总预算、探测状态竞争、读空闲不触发共享压力、候选饥饿和空补丁覆盖属 `model_orchestration` / `deterministic_postprocess`。
- implemented_repairs: 时钟从供应商入场开始，总预算按调用数/并发动态计算并保留答案阶段；任务创建前验证本次选择的精确模型路由，读取超时/模型过载只进入当前任务的服务商+模型+协议熔断，明确 429/并发限制才冷却供应商共享通道；一次成功立即恢复且不相关失败不清除传输故障证据；失败探针终止依赖阶段并向用户报告供应商/模型及责任；失败占位不计入有效答案覆盖，零产出路由阻断不再显示为带问题完成；候选按知识点轮询，空扩展保留已确认证据。
- required_verification: Python 3.11 完整质量门已通过（2271 passed，覆盖率 72%）；已用可控假供应商验证单探针、读空闲降并发、终止传播与用户文案。待用户机恢复连通后再做隔离数据目录的真实启动/部署验证；未授权自动更换用户选定的服务商或重跑历史任务。
