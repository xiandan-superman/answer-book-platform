...
本项目是可独立运行的本地真题解析、知识点出题和专项练习平台。`APP_VERSION` 是用户可见正式版本号的唯一来源，`VERSION` 与发布清单必须保持一致；当前版本为 0.9.25。
...
2. 双击 `start_platform_windows.bat`，无需再区分普通启动脚本和局域网启动脚本。
...
- 每次使用都双击原程序文件夹中的 `start_platform.command`（macOS）或 `start_platform_windows.bat`（Windows），不要直接打开 HTML 文件。启动窗口内可选择仅本机使用或局域网监控；旧的 `start_platform_lan.command` 和 `start_platform_lan_windows.bat` 仅作为兼容入口保留，也会进入同一套依赖检查和图形启动流程。
