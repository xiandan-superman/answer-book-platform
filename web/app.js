...
let activeTaskAnalysisProfile = "evidence_backed";
...
function startTextbookEvidenceTool() {
  currentExamAnalysisProfile = "textbook_evidence_only";
  goToPage("env");
}
...
function syncEnvironmentAnalysisMode() {
  ...
  setText(
    ...
    evidenceOnly
      ? "只选择题目识别、知识点与教材依据模型；不调用答案生成和生图。"
      : questionOnly
      ? "选择结构化解析、正确性复核、读图和生图模型；题目解析不使用教材证据模型。"
      : "分别选择教材依据与题目解析使用的模型和思考深度。"
  );
  setText("examModelSelectionTitle", evidenceOnly ? "本次教材引用定位使用的模型" : questionOnly ? "本次题目解析使用的模型" : "本次真题解析使用的模型");
  setText(
    "examModelSelectionDescription",
    evidenceOnly
...
function syncExamPreparationMode() {
  ...
  setText("examPageEyebrow", evidenceOnly ? "辅助工具 · 教材引用定位" : questionOnly ? "辅助工具 · 题目解析" : "真题解析 · 准备材料");
  setText("examPageTitle", evidenceOnly ? "选择试题与已预处理教材" : questionOnly ? "选择需要解析的题目" : "确认本次解析材料");
  setText(
    "examPageDescription",
    evidenceOnly
      ? "仅生成逐题知识点和已核验教材页码，不生成答案或教材原文。"
      : questionOnly
  ...
  if (createButton) {
    createButton.innerHTML = `${evidenceOnly ? "确认材料并开始定位" : questionOnly ? "确认题目并开始解析" : "确认材料并开始解析"}<i class="fas fa-arrow-right"></i>`;
  }
...
function goToPage(page) {
  ...
  taskNavigationVersion += 1;
  if (page !== "task") {
    cancelActiveReviewDecisionModal?.();
...
function switchResultTab(tab) {
  ...
  $("tab-result-analysis")?.setAttribute("aria-selected", tab === "analysis" ? "true" : "false");
  $("tab-result-review")?.setAttribute("aria-selected", tab === "review" ? "true" : "false");
}

function setResultPageState(state = "empty", task = null) {
  const hasContent = state !== "empty";
  $("resultEmptyState")?.classList.toggle("hidden", hasContent);
  $("resultContent")?.classList.toggle("hidden", !hasContent);
  const icon = $("resultStateIcon");
  if (icon) {
    icon.className = `result-icon ${state === "completed" ? "result-icon-complete" : state === "issues" ? "result-icon-warning" : state === "loading" ? "result-icon-loading" : "result-icon-neutral"}`;
    icon.innerHTML = state === "completed"
  ...
  if (state === "empty") {
    setText("resultPageTitle", "解析结果");
    setText("resultPageSubtitle", "选择一个已完成的任务后查看解析内容和交付文件。");
    return;
  }
  if (state === "loading") {
    setText("resultPageTitle", "正在读取解析结果");
    setText("resultPageSubtitle", "正在同步题目、质量检查和交付文件，请稍候。");
    return;
  }
  const taskName = shortName(task?.exam_path || "解析结果");
  setText("resultPageTitle", `${taskName} · 解析结果`);
...
function renderResultDeliveryVerdict(task = {}, report = null) {
  ...
  if (dataFormalAcceptancePassed(data)) {
    ...
    title = "可预览，但需要复核";
    message = "结果文件已保留，正式交付包仍可下载；使用前请留意下方复核提示。";
...
function visibleTaskStageGroups() {
  if (activeTaskAnalysisProfile === "textbook_evidence_only") {
    return taskStageGroups.filter((group) => ["prepare", "understand", "evidence"].includes(group.key));
  }
  return activeTaskAnalysisProfile === "question_only"
...
function renderTaskStepList(currentStage = "", status = "") {
  ...
  for (const [index, group] of visibleTaskStageGroups().entries()) {
    ...
    else if (status === "failed" && index === currentIndex) item.classList.add("failed");
...
function syncExamModelTestAvailability() {
  ...
  button.title = ready
    ? (currentExamAnalysisProfile === "question_only" ? "实际调用当前结构化解析模型" : currentExamAnalysisProfile === "textbook_evidence_only" ? "实际调用题目识别和教材依据模型" : "实际调用当前知识识别和结构化解析模型")
    : (currentExamAnalysisProfile === "question_only" ? "请先选择结构化解析模型并配置对应 API Key" : currentExamAnalysisProfile === "textbook_evidence_only" ? "请先配置题目识别、教材依据模型及对应 API Key" : "请先配置知识识别、结构化解析模型及对应 API Key");
  return ready;
}
...
async function testExamModelRoutes() {
  ...
  if (button) {
    ...
    button.innerHTML = '<i class="fas fa-circle-notch fa-spin"></i>正在测试当前模型';
  }
  setText("examModelTestHint", `正在测试 ${routes.length} 条实际模型路线，请稍候…`);
  try {
    const results = await Promise.all(routes.map(async (route) => {
      try {
        ...
        const message = providerErrorAdvice(error).body;
        rememberModelConnectionTest(route.provider, route.model, false, message);
    ...
    if (failed.length) {
      setText("examModelTestHint", `${failed.map((item) => item.route.label).join("、")}调用失败，请检查配置后重试。`);
      await platformAlert(failed.map((item) => `${item.route.label}：${item.message}`).join("\n"), { title: "当前模型组合未全部通过", tone: "warning" });
    } else {
      setText("examModelTestHint", `已验证 ${routes.map((route) => `${displayProviderName(route.provider)} / ${route.model}`).join("；")}`);
    }
  } finally {
    if (button) {
...
function updateTaskSummary(task) {
  ...
  const questionOnly = activeTaskAnalysisProfile === "question_only";
  const evidenceOnly = activeTaskAnalysisProfile === "textbook_evidence_only";
  ...
  setText("taskPageEyebrow", `${evidenceOnly ? "教材引用定位" : questionOnly ? "题目解析" : "真题解析"} · ${taskStateLabel}`);
...
function applyExamTaskControls(task = {}, qualitySummary = {}) {
  ...
  if (acceptanceButton) {
    ...
    acceptanceButton.innerHTML = report
      ? '<i class="fas fa-clipboard-check"></i>查看检查结果'
      : '<i class="fas fa-clipboard-check"></i>检查下载条件';
  }
...
function syncProviderControls(providers) {
  ...
  const taskEntries = configuredTaskProviderEntries("text", "answer");
  select.innerHTML = "";
  for (const [name, cfg] of taskEntries) {
  ...
  if (!taskEntries.length) {
    const option = document.createElement("option");
    option.value = "";
    option.textContent = "暂无可用模型，请先完成 API 配置";
    select.appendChild(option);
    select.disabled = true;
  } else {
    select.disabled = false;
  }
  const configuredProvider = taskEntries[0]?.[0];
  if (previousProvider && taskEntries.some(([name]) => name === previousProvider)) select.value = previousProvider;
  else if (configuredProvider) select.value = configuredProvider;
  updateModelControls();
  initializeExamProgressiveLayout();
...
function normalizeBarePracticeLatexCommands(value) {
  const preserved = [];
  let source = String(value || "").replace(/(\\\([\s\S]*?\\\)|\\\[[\s\S]*?\\\]|\$\$[\s\S]*?\$\$|\$[^$\n]+\$)/g, (match) => {
    preserved.push(match);
    return `@@PRESERVED_MATH_${preserved.length - 1}@@`;
  });
  source = source.replace(
    /\\boldsymbol\s*(?:\{[^{}\n]+\}|[A-Za-zΑ-ωΔΣΠΩμνρλθξ])(?:\s*[_^]\s*(?:\{[^{}\n]+\}|[-+A-Za-z0-9Α-ωΔΣΠΩμνρλθξ]+))*/g,
    (match) => `\\(${match.trim()}\\)`
  );
...
function renderPracticeResults(data) {
  ...
  setPracticeStageDescription(successfulCount > 0
    ? "练习结果已保留，可继续下载、编辑或回到蓝图调整。"
    : "题目尚未生成，可查看失败原因并继续未完成项。");
  ...
  const resultModeLabel = completion.primary_code === "generation_incomplete"
    ? (successfulCount > 0 ? "部分题目已生成" : "题目尚未生成")
    : completion.primary_code === "configuration_blocked"
      ? (successfulCount > 0 ? "已有结果已保留，等待配置" : "暂未生成题目，等待配置")
      : completion.primary_code === "review_required"
        ? "题目已生成，等待复核"
        : (strategyLabels[data.generation_strategy] || "专项练习已生成");
  setText("practiceResultMode", resultModeLabel);
  const trainingGoal = data.blueprint?.training_goal || "专项练习";
  setText("practiceTrainingGoal", trainingGoal);
  if ($("practiceTrainingGoal")) $("practiceTrainingGoal").title = trainingGoal;
  ...
  const secondaryIssues = completion.issues.filter((item) => (
    item.code !== primaryIssue.code && !(successfulCount === 0 && item.code === "review_required")
  ));
  const secondarySummary = secondaryIssues.length
    ? `另有：${secondaryIssues.map((item) => item.label).join("；")}。`
    : "";
  const secondaryReasons = secondaryIssues
    .flatMap((item) => Array.isArray(item.reasons) ? item.reasons : [])
    .map((reason) => String(reason || "").trim())
    .filter(Boolean)
    .slice(0, 4);
  const secondaryDetails = secondaryReasons.length
    ? `<details class="practice-quality__details"><summary>查看 ${secondaryReasons.length} 项复核提示</summary><ul>${secondaryReasons.map((reason) => `<li>${escapeHtml(reason)}</li>`).join("")}</ul></details>`
    : "";
  const primaryReasons = (primaryIssue.reasons || []).slice(0, 4).join("；");
  const qualityPanel = ({ icon, title, description, actions = "", details = "" }) => `
    <i class="${escapeHtml(icon)}" aria-hidden="true"></i>
    <div class="practice-quality__content"><strong>${escapeHtml(title)}</strong><p>${description}</p>${details}</div>
    ${actions ? `<div class="practice-quality__actions">${actions}</div>` : ""}
  `;
  $("practiceQuality").innerHTML = primaryIssue.code === "completed"
    ? qualityPanel({ icon: "fas fa-circle-check", title: `${successfulCount} / ${resultCount} 题可用`, description: "可直接下载全部 Word；如只需要部分题目，请先勾选后再下载。" })
    : primaryIssue.code === "configuration_blocked"
      ? qualityPanel({ icon: "fas fa-key", title: "需要检查 API 配置", description: `共 ${resultCount} 题：已生成 ${successfulCount} 题，${unfinishedCount} 题待配置。${escapeHtml(secondarySummary)}`, details: secondaryDetails, actions: `<button type="button" class="secondary-button" data-practice-config><i class="fas fa-key"></i>检查 API 配置</button>${data.history_id && generationIncomplete ? '<button type="button" class="primary-button" data-practice-continue><i class="fas fa-arrow-right"></i>继续未完成项</button>' : ""}` })
      : primaryIssue.code === "generation_incomplete"
      ? qualityPanel({ icon: "fas fa-triangle-exclamation", title: "存在未完成题目", description: `共 ${resultCount} 题：已生成 ${successfulCount} 题，${unfinishedCount} 题未完成。${escapeHtml(secondarySummary)}`, details: secondaryDetails, actions: data.history_id ? '<button type="button" class="primary-button" data-practice-continue><i class="fas fa-arrow-right"></i>继续未完成项</button>' : "" })
      : primaryIssue.code === "review_required"
      ? qualityPanel({ icon: "fas fa-triangle-exclamation", title: "题目已生成 · 待复核", description: `${escapeHtml(primaryReasons || "结果已保留，请复核后使用。")} 结果可继续查看和编辑。` })
      : qualityPanel({ icon: "fas fa-circle-info", title: "已完成 · 有提示", description: `${escapeHtml(primaryReasons || "结果含非阻断提示，请查看后使用。")} 题目已完整保留。` });
  ...
  $("practiceStatusBanner")?.classList.add("is-result-summary");
  $("practiceStatusBanner")?.setAttribute("aria-hidden", "true");
      ...
      <header class="practice-exercise__header">
        ...
        <div class="practice-exercise__meta">
          ...
          <div class="practice-exercise__actions">
            ...
            ${generationFailed ? "" : `<div class="practice-action-menu practice-action-menu--question" data-practice-action-menu>
              ...
              <div class="practice-action-menu__panel hidden" role="menu" data-practice-menu-panel>
                <button type="button" class="practice-action-menu__item" role="menuitem" data-practice-copy="${idx}"><i class="far fa-copy"></i><span>复制本题</span></button>
                <button type="button" class="practice-action-menu__item" role="menuitem" data-practice-download="${idx}" data-practice-word-export-key="${escapeHtml(wordExportKey)}" data-practice-word-export-label="下载本题 Word" data-practice-word-export-available="true"><i class="fas fa-file-word"></i><span>下载本题 Word</span></button>
              </div>
            </div>`}
          </div>
        </div>
...
function practiceRequestPayload() {
  ...
  return {
    ...
    thinking: selectedTaskThinkingMode(knowledgeMode ? "knowledge" : "practice")
  };
}
...
function knowledgeRequestPayload() {
  ...
  return {
    ...
    thinking: selectedTaskThinkingMode("knowledge")
  };
}
...
function renderPracticePlan(plan) {
  ...
  setText("practicePlanGoal", "确认训练蓝图");
  ...
  $("practicePlanList").innerHTML = planItems.map((item, index) => `
    <details class="practice-plan-edit-row" data-plan-index="${index}"${index === 0 ? " open" : ""}>
      ...
      <div class="practice-plan-edit-body"><div class="practice-plan-edit-grid">
        ...
        <details class="practice-plan-wide practice-plan-source-disclosure"><summary><span>来源绑定</span><small>已选 ${(item.source_refs || [item.source_question_id]).filter(Boolean).length} 项 · 点击展开修改</small></summary><fieldset class="practice-plan-source-editor"><legend class="sr-only">来源绑定</legend>${sourceCatalog.map((source) => {
        ...
        }).join("") || "<span>当前流程无需来源绑定</span>"}</fieldset></details>
      </div>
...
function exportablePracticeSet() {
  if (!latestPracticeSet) return null;
  const exercises = (latestPracticeSet.exercises || []).filter((item) => item && item.generation_status !== "failed");
...
function updatePracticeSelectionActions() {
  ...
  const fullData = exportablePracticeSet();
  ...
  syncPracticeWordExportButton(
    $("practiceDownloadAllBtn"),
    fullData ? practiceWordExportKey(fullData, practiceWordFilename(fullData, `全部${fullData.exercises.length}题`)) : "",
    Boolean(fullData),
    "下载全部 Word"
  );
...
function applyExamLibraryFilters() {
  const cards = Array.from(document.querySelectorAll("#examCardList .exam-card"));
  const query = examLibrarySearchQuery.trim().toLocaleLowerCase("zh-CN");
  let matched = 0;
  let visible = 0;
  cards.forEach((card, index) => {
    const matches = !query || String(card.dataset.search || "").includes(query);
    if (matches) matched += 1;
    const withinPreview = Boolean(query) || examLibraryExpanded || index < EXAM_LIBRARY_PREVIEW_LIMIT;
    const show = matches && withinPreview;
    card.classList.toggle("hidden", !show);
    if (show) visible += 1;
  });
  setText("examLibrarySummary", cards.length ? `显示 ${visible} / ${cards.length} 份真题` : "暂无真题");
  $("examLibraryEmpty")?.classList.toggle("hidden", cards.length === 0 || matched > 0);
  const toggle = $("examLibraryToggle");
  const hasOverflow = !query && cards.length > EXAM_LIBRARY_PREVIEW_LIMIT;
  if (toggle) {
    toggle.classList.toggle("hidden", !hasOverflow);
    toggle.innerHTML = examLibraryExpanded
      ? '<i class="fas fa-chevron-up"></i>收起文件列表'
      : `<i class="fas fa-chevron-down"></i>查看其余 ${cards.length - EXAM_LIBRARY_PREVIEW_LIMIT} 份真题`;
    toggle.setAttribute("aria-expanded", String(examLibraryExpanded));
  }
}

function searchExamLibrary(value) {
  examLibrarySearchQuery = String(value || "");
  applyExamLibraryFilters();
}

function toggleExamLibrary() {
  examLibraryExpanded = !examLibraryExpanded;
  applyExamLibraryFilters();
}
  ...
  if (!(libraryFiles.exams || []).length) {
    ...
    for (const [index, file] of (libraryFiles.exams || []).entries()) {
      ...
      if (examCardList) {
        ...
        card.dataset.search = `${file.name || ""} ${file.path || ""}`.toLocaleLowerCase("zh-CN");
        card.innerHTML = `
          <button type="button" class="exam-card-select" data-action="select-file" aria-label="选择真题 ${escapeHtml(file.name)}">
          ...
          <button type="button" class="library-delete-icon" data-action="delete-file" title="删除 ${escapeHtml(file.name)}" aria-label="删除真题 ${escapeHtml(file.name)}"><i class="fas fa-trash"></i></button>
        `;
        card.querySelector("[data-action='select-file']")?.addEventListener("click", () => {
  ...
  if (readiness) {
    readiness.textContent = uploading ? "正在处理真题文件，请稍候。" : questionOnly
      ...
      : !examPath && !textbookCount
        ? "请选择一份真题和至少一本教材。"
        : !examPath
          ? `已选择 ${textbookCount} 本教材，还需选择一份真题。`
          : "已选择真题，还需选择至少一本教材。";
  }
  readinessBox?.classList.toggle("is-ready", ready);
}
...
function selectedVisionProviderConfig() {
  const name = $("visionProviderSelect")?.value || "";
  return name ? (providerConfigs[name] || {}) : {};
}

function selectedImageProviderConfig() {
  const name = $("imageProviderSelect")?.value || "";
  return name ? (providerConfigs[name] || {}) : {};
}
...
function updateModelCapabilityRisk() {
  ...
  const imageLabel = displayProviderName(imageCfg.name || $("imageProviderSelect")?.value || "生图模型");
  if (currentExamAnalysisProfile === "textbook_evidence_only") {
    if (!providerHasVision(visionCfg)) {
      genericRisks.push(`${visionLabel} 当前未配置多模态视觉模型；有图试题的知识点定位可能不完整。`);
    }
...
const TASK_SUPPORT_OK = new Set(["recommended", "allowed", "limited", "unknown"]);

function registeredModelProfile(cfg, model) {
  const value = String(model || "").trim();
  const profile = value ? (cfg?.model_profiles || {})[value] : null;
  return profile && typeof profile === "object" ? profile : null;
}

function modelTaskSupportIsUsable(profile, keys) {
  const support = profile?.task_support;
  if (!support || typeof support !== "object") return false;
  const values = keys.map((key) => String(support[key] || "").toLowerCase()).filter(Boolean);
  if (!values.length || values.includes("forbidden")) return false;
  return values.every((value) => TASK_SUPPORT_OK.has(value));
}

function taskPurposeSupportKeys(kind, purpose = "") {
  if (kind === "image") return ["image_generation"];
  if (kind === "vision") return ["source_analysis"];
  if (purpose === "reasoning") return ["blueprint", "review"];
  if (purpose === "correctness") return ["review"];
  if (purpose === "practice" || purpose === "knowledge") return ["blueprint", "generation"];
  return ["answer"];
}
...
function registeredModelSupportsKind(cfg, model, kind, purpose = "") {
  ...
  return String(profile.kind || "text_generation").toLowerCase() !== "image_generation";
}
...
function providerEntriesByCapability(kind, purpose = "") {
  return configuredTaskProviderEntries(kind === "all" ? "text" : kind, purpose);
}
...
function modelLooksVisionCapable(model, cfg = currentProviderConfig()) {
  ...
  const explicitCapabilities = Array.isArray((cfg.model_capabilities || {})[value])
    ...
    : [];
  const profile = registeredModelProfile(cfg, value);
  const nativeInputs = Array.isArray(profile?.native_inputs)
    ? profile.native_inputs.map((input) => String(input).toLowerCase())
    : [];
...
function populateProviderSelect(selectId, kind, preferredName, purpose = "") {
  ...
  if (!select) return "";
  const entries = providerEntriesByCapability(kind, purpose);
  select.innerHTML = "";
  if (!entries.length) {
    const option = document.createElement("option");
    option.value = "";
...
function selectedTextRoleProviderConfig(roleKey) {
  ...
  const name = role ? ($(role.providerId)?.value || "") : "";
  return name ? (providerConfigs[name] || {}) : {};
}
...
function populateTextRoleModelSelect(roleKey, preferredModel = "") {
  ...
  const options = taskModelOptions("text", cfg, roleKey);
  ...
  if (options.length) {
    ...
    const option = document.createElement("option");
    option.value = "";
    option.textContent = "当前供应商没有适合本环节的模型";
    select.appendChild(option);
  }
  select.hidden = false;
...
function updateTextRoleHint(roleKey) {
  ...
  const modelName = selectedTextRoleModel(roleKey) || cfg.default_model || "";
  const modelLabel = readableModelLabel(modelName, cfg);
  const summary = `${providerName} · ${modelLabel}`;
  hint.innerHTML = `<i class="fas ${role.icon}"></i><span title="${escapeHtml(role.label)}：${escapeHtml(summary)}">${escapeHtml(summary)}</span>`;
  updateModelRoleCards();
}
...
function updateTextRoleModelControls() {
  ...
  for (const roleKey of Object.keys(textModelRoles)) {
    const role = textModelRoles[roleKey];
    populateProviderSelect(role.providerId, "text", currentName, roleKey);
...
function syncExamFollowerRolesFromAnswer() {
  ...
  setExamTextRoleRoute("correctness", [providerName, modelName]);
}
...
function populateVisionModelSelect(preferredModel = "") {
  ...
  if (!select || !input) return;
  const options = taskModelOptions("vision", cfg, "source_analysis");
  const labels = cfg.model_option_labels || {};
  select.innerHTML = "";
  if (!options.length) {
    const option = document.createElement("option");
    option.value = "";
    option.textContent = "当前没有可用的识图模型";
    select.appendChild(option);
  } else {
    for (const model of options) {
      const option = document.createElement("option");
...
function populateImageModelControls(preferredModel = "") {
  ...
  if (!input) return;
  const options = taskModelOptions("image", cfg, "image_generation");
  ...
  if (select) {
    ...
    if (!options.length) {
      const option = document.createElement("option");
      option.value = "";
      option.textContent = "当前没有可用的生图模型";
      select.appendChild(option);
    } else {
      for (const model of options) {
        const option = document.createElement("option");
...
function updateCapabilityModelHints() {
  ...
  if (visionHint) {
    const visionProviderName = displayProviderName(visionCfg.name || $("visionProviderSelect")?.value);
    const visionModelLabel = readableModelLabel(selectedVisionModel() || visionCfg.vision_model, visionCfg);
    visionHint.innerHTML = providerHasVision(visionCfg)
      ? `<i class="fas fa-eye"></i><span title="有图题读图：${escapeHtml(visionProviderName)} · ${escapeHtml(visionModelLabel)}">${escapeHtml(visionProviderName)} · ${escapeHtml(visionModelLabel)}</span>`
      : '<i class="fas fa-triangle-exclamation"></i>当前服务商未配置多模态识图模型。';
  }
  const imageHint = $("imageModelHint");
  if (imageHint) {
    const imageProviderName = displayProviderName(imageCfg.name || $("imageProviderSelect")?.value);
    const imageModelLabel = readableModelLabel(selectedImageModel() || imageCfg.image_model, imageCfg, "image");
    imageHint.innerHTML = providerHasImageModel(imageCfg)
      ? `<i class="fas fa-image"></i><span title="生图路线：${escapeHtml(imageProviderName)} · ${escapeHtml(imageModelLabel)}">${escapeHtml(imageProviderName)} · ${escapeHtml(imageModelLabel)}</span>`
...
function updateCapabilityModelControls() {
  const currentName = $("providerSelect")?.value || "";
  populateProviderSelect("visionProviderSelect", "vision", currentName, "source_analysis");
  populateProviderSelect("imageProviderSelect", "image", "lingsuan_image", "image_generation");
...
function updateModelControls() {
  const cfg = currentProviderConfig();
  const options = taskModelOptions("text", cfg, "answer");
  ...
  if (options.length) {
    ...
    modelSelect.value = options.includes(cfg.default_model) ? cfg.default_model : options[0];
  ...
  const option = document.createElement("option");
  option.value = "";
  option.textContent = "当前供应商没有适合解析任务的模型";
  modelSelect.appendChild(option);
  modelSelect.hidden = false;
...
function keyProviderModelCatalog(cfg) {
  ...
  const models = Array.from(new Set([...textModels, ...imageModels]));
  const textLabels = cfg.model_option_labels || {};
  const imageLabels = cfg.image_model_option_labels || {};
  if (!models.length) return '<span class="key-model-catalog-empty">由供应商默认模型决定</span>';
  return models.map((model) => {
    const profile = registeredModelProfile(cfg, model);
    const isImage = String(profile?.kind || "").toLowerCase() === "image_generation" || imageModels.includes(model);
    const readsImages = modelLooksVisionCapable(model, cfg);
    const capability = isImage ? "生图" : readsImages ? "图文" : "文本";
    const label = imageLabels[model] || textLabels[model] || model;
    return `<span class="key-model-chip ${isImage ? "image" : readsImages ? "vision" : "text"}" title="${escapeHtml(model)}"><em>${escapeHtml(capability)}</em>${escapeHtml(label)}</span>`;
  }).join("");
}
...
function buildApiProviderNavigation(entries) {
  ...
  const take = (names) => names.filter((name) => byName.has(name)).map((name) => [name, byName.get(name)]);
  ...
  const configuredIds = new Set(catalog
    .filter((entry) => entry.entries.some(([, cfg]) => cfg.api_key_set))
    .map((entry) => entry.id));
  const configured = catalog
    .filter((entry) => configuredIds.has(entry.id))
    .map((entry) => ({ ...entry, id: `configured:${entry.id}`, section: "已配置" }));
  return [...configured, ...catalog.filter((entry) => !configuredIds.has(entry.id))];
}
...
function renderKeyProviderCards() {
  ...
  const entries = Object.entries(providerConfigs || {}).filter(([name]) => !HIDDEN_API_CONFIG_PROVIDER_NAMES.has(name)).sort((left, right) => {
  ...
  const navigation = buildApiProviderNavigation(entries);
  const selectedItem = navigation.find((entry) => entry.id === expandedKeyProviderName) || navigation[0];
  expandedKeyProviderName = selectedItem.id;
    ...
    <form class="key-provider-card${expanded ? " expanded" : ""}" data-key-provider="${escapeHtml(name)}" data-key-label="${escapeHtml(displayProviderName(name))}" data-key-capabilities="${escapeHtml(keyProviderCapabilityText(cfg))}" data-key-models="${escapeHtml([...(cfg.model_options || []), ...(cfg.image_model_options || []), cfg.default_model, cfg.image_model].filter(Boolean).join(" "))}" data-key-saved="${cfg.api_key_set ? "true" : "false"}" autocomplete="off">
      <header>
        <button type="button" class="key-provider-summary" data-key-card-toggle="${escapeHtml(name)}" aria-expanded="${expanded ? "true" : "false"}" aria-controls="key-details-${escapeHtml(name)}">
          ...
          <span class="key-provider-summary__copy">
            <strong>${escapeHtml(displayProviderName(name))}</strong>
            <small>${escapeHtml(keyProviderCapabilityText(cfg))}</small>
          </span>
          ...
          <i class="fas fa-chevron-down key-provider-summary__chevron" aria-hidden="true"></i>
        </button>
      </header>
      <div id="key-details-${escapeHtml(name)}" class="key-provider-details${expanded ? "" : " hidden"}" data-key-card-details>
        ...
        <div class="key-model-catalog"><strong>该通道支持的模型</strong><div>${keyProviderModelCatalog(cfg)}</div></div>
  ...
  grid.querySelectorAll("[data-key-group]").forEach((button) => button.addEventListener("click", () => { expandedKeyProviderName = button.dataset.keyGroup || ""; renderKeyProviderCards(); }));
  grid.querySelector("[data-key-config-retry]")?.addEventListener("click", () => loadApiConfiguration());
  grid.querySelector("[data-key-config-recover]")?.addEventListener("click", recoverDamagedApiConfiguration);
  grid.querySelectorAll("form[data-key-provider]").forEach((form) => {
    form.addEventListener("submit", (event) => event.preventDefault());
  });
  grid.querySelectorAll("[data-key-card-toggle]").forEach((button) => button.setAttribute("aria-expanded", "true"));
...
function taskThinkingControlIds(profile) {
  const prefix = profile === "knowledge" ? "knowledge" : "practice";
  return {
    select: `${prefix}ThinkingModeSelect`,
    hint: `${prefix}ProtocolHint`,
  };
}
...
function defaultTaskProvider(kind, purpose = "practice") {
  const preferredProvider = kind === "image" ? "lingsuan_image" : "lingsuan_google";
  const entries = configuredTaskProviderEntries(kind, purpose);
...
function taskProviderName(profile, kind) {
  const ids = taskModelControlIds(profile, kind);
  const entries = configuredTaskProviderEntries(kind, profile);
  ...
  return available.has(saved) ? saved : defaultTaskProvider(kind, profile);
}
...
function taskModelOptions(kind, cfg, purpose = "") {
  if (kind === "image") {
    return Array.from(new Set([
      cfg.image_model,
      ...(Array.isArray(cfg.image_model_options) ? cfg.image_model_options : []),
    ].filter(Boolean))).filter((model) => registeredModelSupportsKind(cfg, model, kind, purpose));
  }
  const candidates = kind === "vision"
    ? [
      cfg.vision_model,
      ...(Array.isArray(cfg.vision_model_options) ? cfg.vision_model_options : []),
      ...(Array.isArray(cfg.model_options) ? cfg.model_options : []),
    ]
    : (Array.isArray(cfg.model_options) ? cfg.model_options : [cfg.default_model]);
...
function populateTaskModelControl(profile, kind, preferredModel = "") {
  ...
  const providerName = providerSelect.value || defaultTaskProvider(kind, profile);
  modelSelect.innerHTML = "";
  const cfg = providerConfigs[providerName] || {};
  const options = practiceModelOptions(kind, cfg, profile);
  const labels = kind === "image" ? (cfg.image_model_option_labels || {}) : (cfg.model_option_labels || {});
  modelSelect.disabled = false;
  if (options.length) {
    for (const model of options) {
    ...
    const option = document.createElement("option");
    option.value = "";
    option.textContent = "当前没有适合此用途的模型";
    modelSelect.appendChild(option);
    modelSelect.disabled = true;
  }
  input.hidden = true;
  ...
  if (kind === "text") {
    ...
    populateThinkingModeControl(
      ...
      providerName,
      modelSelect.value,
      savedTaskModelSetting(profile, "text").thinking || ""
    );
  }
}
...
function populateTaskModelSettings(profile) {
  for (const kind of ["text", "vision", "image"]) {
    ...
    const previousProvider = select.value || saved.provider || defaultTaskProvider(kind, profile);
    ...
    select.value = entries.some(([name]) => name === previousProvider) ? previousProvider : (defaultTaskProvider(kind, profile) || "");
...
function selectedTaskModel(profile, kind) {
  ...
  const selected = $(ids.model)?.value || "";
  if (options.includes(selected)) return selected;
  if (options.includes(configuredDefault)) return configuredDefault;
  return options[0] || "";
}
...
function selectedTaskThinkingMode(profile) {
...
function updateTaskModelSummary(profile) {
  ...
  setText(
    ...
    primaryHandlesImages
      ? `主模型已能读图；备用：${displayProviderName(visionProviderName || "未选择")} · ${visionModelLabel}（通常不调用）`
      : `有图材料启用：${displayProviderName(visionProviderName || "未选择")} · ${visionModelLabel}${visionKeyState}`
  );
  setText(
    taskModelControlIds(profile, "image").summary,
    imageKeyState
      ? `当前生图服务缺少 API Key：${displayProviderName(imageProviderName || "未选择")}`
      : "生图路线已配置"
  );
  if (profile === "practice" || (profile === "knowledge" && currentPracticeSourceMode === "knowledge")) {
    setText("practiceCurrentModelBadge", `${shortTaskModelName(textModel, textProviderName)} · ${thinkingLabel}${textKeyState}`);
  }
  if (profile === "knowledge") {
    setText("knowledgeModelSummary", `${shortTaskModelName(textModel, textProviderName)} · ${thinkingLabel}${textKeyState}`);
  }
  syncTaskProgressiveModelUi(profile, {
    textModel,
    textProvider,
    primaryHandlesImages,
  });
...
function syncTaskProgressiveModelUi(profile, state = {}) {
  const prefix = profile === "knowledge" ? "knowledge" : "practice";
...
const THINKING_MODE_ORDER = ["disabled", "minimal", "low", "medium", "high", "xhigh"];

function modelRequestProtocol(cfg, model) {
...
function supportedThinkingModes(cfg, model) {
  const profile = (cfg?.model_profiles || {})[String(model || "").trim()] || {};
  const explicit = Array.isArray(profile.supported_thinking_modes)
    ? profile.supported_thinking_modes.map((value) => String(value || "").trim().toLowerCase())
      .filter((value) => Object.hasOwn(THINKING_MODE_LABELS, value))
    : [];
  if (explicit.length) return Array.from(new Set(explicit));
  const minimum = String(profile.thinking_minimum || "").trim().toLowerCase();
  const minimumIndex = THINKING_MODE_ORDER.indexOf(minimum);
  if (minimumIndex >= 0) return ["auto", ...THINKING_MODE_ORDER.slice(minimumIndex)];
  if (profile.thinking === "not_applicable") return ["disabled"];
  // Existing routes predate the per-model admission contract. Keep them
  // usable until independently re-verified; new routes must provide the
  // explicit field before they are published.
  return ["auto", "disabled", "low", "medium", "high", "xhigh"];
}

function populateThinkingModeControl(selectId, hintId, providerName, model, preferred = "") {
  const select = $(selectId);
  if (!select) return "auto";
  if (!String(model || "").trim()) {
    select.innerHTML = '<option value="auto">请先选择模型</option>';
    select.disabled = true;
    setText(hintId, "选择模型后显示已登记的请求类型和可用思考深度。");
    return "auto";
  }
  const cfg = providerConfigs?.[providerName] || {};
  const profile = (cfg.model_profiles || {})[String(model || "").trim()] || {};
...
function populateRoleThinkingMode(roleKey, preferred = "") {
  const role = textModelRoles[roleKey];
  if (!role?.thinkingSelectId) return "auto";
  const cfg = selectedTextRoleProviderConfig(roleKey);
  return populateThinkingModeControl(
    role.thinkingSelectId,
    role.protocolHintId,
    cfg.name || $(role.providerId)?.value || "",
...
function selectedRoleThinkingMode(roleKey) {
  const role = textModelRoles[roleKey];
  return selectedThinkingMode(role?.thinkingSelectId || "thinkingModeSelect");
}

function selectedThinkingMode(selectId = "thinkingModeSelect") {
  const value = $(selectId)?.value || "auto";
  return Object.hasOwn(THINKING_MODE_LABELS, value) || value === "enabled" ? value : "auto";
}
...
async function createTask() {
  ...
  setVisual(
    "taskVisualResult",
    evidenceOnly ? "正在创建教材引用定位任务" : questionOnly ? "正在创建题目解析任务" : "正在创建真题项目",
    questionOnly ? "平台将只处理题目，不读取教材。" : "平台会检查所选教材是否已有可复用索引。",
    "info"
  );
  try {
...
function readableModelLabel(value, cfg = {}, kind = "text") {
  const model = String(value || "").trim();
  if (!model) return "未选择模型";
  const labels = kind === "image" ? (cfg.image_model_option_labels || {}) : (cfg.model_option_labels || {});
  return String(labels[model] || shortTaskModelName(model, cfg.name || ""));
}
...
function renderTaskManager(tasks = latestTasks) {
  ...
  const filteredVisible = sortedTasks(tasks.filter((task) => {
    ...
    return statusMatch && (activeTaskKind === "all" || kind === activeTaskKind) && queryMatch;
  }));
  ...
  const showTaskLoading = taskManagerLoading && tasks.length === 0;
  const hasNoTasks = !showTaskLoading && tasks.length === 0;
  const emptyKindCreation = !showTaskLoading
    && filteredVisible.length === 0
    && !activeTaskQuery.trim()
  ...
  if (empty) {
    ...
    if (action && !showTaskLoading) {
      ...
      action.dataset.emptyKind = emptyKindCreation;
      const actionLabel = emptyKindCreation ? creationLabels[emptyKindCreation] : hasNoTasks ? "选择任务类型" : "清除筛选与搜索";
  ...
  for (const [index, task] of visible.entries()) {
    ...
    const kindMeta = {
      exam: {
        label: task.analysis_profile === "textbook_evidence_only" ? "教材引用定位" : task.analysis_profile === "question_only" ? "题目解析" : "真题解析",
        icon: task.analysis_profile === "question_only" ? "fas fa-file-lines" : task.analysis_profile === "textbook_evidence_only" ? "fas fa-book-bookmark" : "fas fa-book-open",
      },
    ...
    const phaseSteps = Array.isArray(task.steps)
      ? task.steps.map((phase) => phase.label || phaseLabels[phase.operation] || phase.operation).filter(Boolean)
      : [];
    const phaseText = phaseSteps.length
      ? phaseSteps.filter((label, phaseIndex) => phaseIndex === 0 || label !== phaseSteps[phaseIndex - 1]).join(" → ")
      : "题目生成";
    const subtitle = formatTask
      ...
      : task.analysis_profile === "question_only"
      ? "不使用教材 · 仅解析题目内容"
      : task.analysis_profile === "textbook_evidence_only"
      ? `仅产出知识点与页码 · 教材：${Array.isArray(task.textbook_material_names) && task.textbook_material_names.length ? task.textbook_material_names.map(shortName).join("、") : "暂无教材信息"}`
    ...
    const displayProgressMessage = normalized === "failed" && progressMessage.length > 120
      ? `${progressMessage.slice(0, 108).replace(/[，。；\s]+$/, "")}… 请点击“处理未完成”查看详情并选择下一步。`
      : progressMessage;
    ...
    const displayCurrentStageText = formatTask
      ...
      : currentStageText;
    const showCurrentStage = String(displayCurrentStageText || "").trim() !== String(meta.label || "").trim();
      ...
      <div class="task-manager-main">
        ...
        <div class="task-manager-copy">
          ...
          <div class="task-manager-meta">
            ...
            ${showCurrentStage ? `<span class="task-current-stage"><i class="fas fa-layer-group"></i>${escapeHtml(displayCurrentStageText)}</span>` : ""}
          </div>
...
function renderSystemStatus(data) {
  ...
  const providerRoutes = $("systemProviderRoutes");
  if (providerRoutes) {
    const gates = Array.isArray(models.provider_gates) ? models.provider_gates : [];
    providerRoutes.innerHTML = gates.length
      ? `<p class="system-provider-route-title">供应商共享通道</p>${gates.map((gate) => {
          const providerName = String(gate.provider || "default");
          const routeLabel = providerName === "lingsuan" ? "灵算 · 共享模型通道" : displayProviderName(providerName);
          const activeModels = (Array.isArray(models.active) ? models.active : [])
            .filter((call) => {
              const activeProvider = String(call.provider || "").toLowerCase();
              return providerName === "lingsuan" ? activeProvider === "lingsuan" || activeProvider.startsWith("lingsuan_") : activeProvider === providerName.toLowerCase();
            })
            .map((call) => String(call.model || ""))
            .filter(Boolean);
          const modelText = [...new Set(activeModels)].join("、") || "暂无运行模型";
          const cooldown = Number(gate.cooldown_remaining_seconds || 0);
          const state = cooldown > 0 ? `冷却 ${formatElapsedSeconds(cooldown)}` : Number(gate.waiting || 0) > 0 ? "正在排队" : "可用";
          return `<div class="system-provider-route${cooldown > 0 ? " is-cooling" : ""}">
            <div><strong>${escapeHtml(routeLabel)}</strong><small>${escapeHtml(modelText)}</small></div>
            <span>运行 ${Number(gate.active || 0)}/${Number(gate.limit || 0)} · 等待 ${Number(gate.waiting || 0)}</span>
            <em>${escapeHtml(state)}</em>
          </div>`;
        }).join("")}`
      : '<div class="system-empty-line">当前还没有受容量控制的模型通道</div>';
  }
...
async function refreshSystemStatusFromButton() {
  const button = $("refreshSystemBtn");
  if (!button || button.disabled) return;
  button.disabled = true;
  ...
  try {
    ...
    button.innerHTML = '<i class="fas fa-sync-alt"></i>刷新运行状态';
  }
}
...
async function refreshStorageOverviewFromButton() {
  const button = $("refreshStorageBtn");
  if (!button || button.disabled) return;
  button.disabled = true;
  button.setAttribute("aria-busy", "true");
  button.innerHTML = '<i class="fas fa-spinner fa-spin"></i>正在统计';
  try {
    await loadStorageOverview();
  } finally {
    button.disabled = false;
    button.removeAttribute("aria-busy");
    button.innerHTML = '<i class="fas fa-sync-alt"></i>重新统计磁盘';
  }
}
...
const STORAGE_ENTRY_PREVIEW_LIMIT = 8;
const storageExpandedKinds = new Set();
...
function renderStorageOverview(overview) {
  ...
  for (const area of overview.areas || []) {
    ...
    const expanded = storageExpandedKinds.has(area.kind);
    const visibleEntries = expanded ? entries : entries.slice(0, STORAGE_ENTRY_PREVIEW_LIMIT);
    ...
    if (entries.length > STORAGE_ENTRY_PREVIEW_LIMIT) {
      const toggle = document.createElement("button");
      toggle.type = "button";
      toggle.className = "storage-area-toggle";
      toggle.dataset.storageExpandKind = area.kind;
      toggle.setAttribute("aria-expanded", String(expanded));
      toggle.innerHTML = expanded
        ? '<i class="fas fa-chevron-up"></i>收起列表'
        : `<i class="fas fa-chevron-down"></i>查看其余 ${entries.length - STORAGE_ENTRY_PREVIEW_LIMIT} 项`;
...
function confirmCleanupStorage(selectedOnly) {
  const grouped = {};
  if (selectedOnly) {
    for (const check of document.querySelectorAll(".storage-entry-check:checked")) {
      (grouped[check.dataset.kind] ||= []).push(check.dataset.id);
    }
  } else {
    for (const area of storageOverviewData?.areas || []) {
      for (const entry of area.entries || []) {
        if (!entry.deletable) continue;
        (grouped[area.kind] ||= []).push(String(entry.id || entry.key));
      }
    }
  }
...
document.addEventListener("DOMContentLoaded", () => {
  $("refreshStorageBtn")?.addEventListener("click", refreshStorageOverviewFromButton);
  $("cleanupStorageBtn")?.addEventListener("click", () => confirmCleanupStorage(false));
  $("storageAreas")?.addEventListener("click", (event) => {
    const toggle = event.target.closest("[data-storage-expand-kind]");
    if (!toggle) return;
    const kind = String(toggle.dataset.storageExpandKind || "");
    if (storageExpandedKinds.has(kind)) storageExpandedKinds.delete(kind);
    else storageExpandedKinds.add(kind);
    renderStorageOverview(storageOverviewData);
    $("storageAreas")?.querySelector(`[data-kind="${CSS.escape(kind)}"]`)?.scrollIntoView({ block: "nearest" });
  });
});
...
function taskManagerActions(task = {}, reviewPending = false) {
  ...
  if (task.is_format_task) {
    ...
    if (task.is_generation_job) {
      ...
      add(caps.view_progress && !caps.view_result, "job-status", "blue-action", task.status === "running" ? "fas fa-spinner fa-spin" : "fas fa-eye", task.status === "failed" ? "处理未完成" : "查看进度");
      add(caps.view_quality && task.status === "failed", "job-status", "red-action", "fas fa-triangle-exclamation", "处理未完成");
      ...
      add(caps.resume, "job-resume", "green-action", "fas fa-arrow-right", "继续");
    ...
    add(caps.resume, "resume", "green-action", "fas fa-arrow-right", "继续");
  ...
  const bounded = actions.slice(0, 5);
  const renderAction = ([action, color, icon, label, disabled = false]) => `<button type="button" class="task-card-button ${color}" data-action="${action}"${disabled ? " disabled" : ""}><i class="${icon}"></i>${label}</button>`;
  const primary = bounded.slice(0, 1).map(renderAction).join("");
...
function generationTaskManagerActions(task = {}) {
  if (task.is_generation_job) {
    ...
    if (task.status === "failed") {
      ...
      return `<button type="button" class="task-card-button red-action" data-action="job-status"><i class="fas fa-triangle-exclamation"></i>处理未完成</button>${configAction}${retryAction}`;
    }
    if (task.status === "paused") {
      return '<button type="button" class="task-card-button blue-action" data-action="job-status"><i class="fas fa-eye"></i>查看进度</button><button type="button" class="task-card-button green-action" data-action="job-resume"><i class="fas fa-arrow-right"></i>继续</button><button type="button" class="task-card-button red-action" data-action="job-cancel"><i class="fas fa-times"></i>取消任务</button>';
    }
...
function clearTaskManagerFilters() {
  ...
  $("taskSearchClearBtn")?.classList.add("hidden");
  filterTasks("all");
  filterTaskKind("all");
}
...
async function openTaskDetail(task, showDiagnostics = false) {
  if (!task?.task_id) return;
  if (activeTaskId && activeTaskId !== task.task_id) {
    cancelActiveReviewDecisionModal?.();
    cancelActiveExamStructureReviewModal?.();
  }
  stopTaskPolling();
  ...
  try {
    ...
    if (navigationVersion !== taskNavigationVersion || currentPage !== "task" || activeTaskId !== task.task_id) return;
    updateTaskSummary(data.task);
    ...
    if (navigationVersion !== taskNavigationVersion || currentPage !== "task" || activeTaskId !== task.task_id) return;
    if (isLiveTask(data.task) || isActionRequiredTask(data.task)) startTaskPolling(task.task_id);
  } catch (err) {
    if (navigationVersion !== taskNavigationVersion || currentPage !== "task" || activeTaskId !== task.task_id) return;
...
async function openTaskResult(task) {
  ...
  await Promise.allSettled([
    ...
    loadReview(task.task_id, navigationVersion),
  ]);
}
...
function publicDiagnosticMessage(message = "", questionId = "") {
  if (message && typeof message === "object") {
    if (!questionId) questionId = message.question_id || message.questionId || "";
    message = message.message || message.detail || message.label || message.code || "";
  }
  let text = String(message || "").trim();
  const embeddedMessage = text.match(/["']message["']\s*:\s*(["'])(.*?)\1(?:\s*,|\s*})/i);
  if (embeddedMessage?.[2]) text = embeddedMessage[2].replace(/\\(["'])/g, "$1").trim();
  if (questionId) {
...
function showExamStructureReviewModal(request, submit = null) {
  ...
  return new Promise((resolve) => {
    ...
    const finish = async (decision) => {
      ...
      cancelActiveExamStructureReviewModal = null;
...
async function checkReviewDecision(taskId) {
  ...
  const allowed = await showReviewDecisionModal({ ...request, task_id: taskId });
  if (allowed === null || currentPage !== "task" || taskId !== activeTaskId) {
    handledReviewDecisionRequests.delete(requestId);
    return;
  }
...
function buildTaskExecutionDetail(task, current, progress, stages) {
  ...
  if (current === "question_understanding" && total) {
    ...
    detail.text = questionOnly
      ? "模型根据题目内容组织答案、公式和图件说明。"
      : "模型将教材依据组织为答案、公式和图件说明。";
    addMetric(`已完成 ${completed}/${total} 题`);
    ...
    detail.text = questionOnly
      ? "核对每道题是否有答案、计算步骤和必要图件。"
      : "核对每道题是否有答案、教材依据、计算步骤和必要图件。";
    ...
    detail.text = questionOnly
      ? "检查答案完整性、计算过程和作图要求；仅把存在问题的题目送入修复。"
      : "检查答案完整性、教材引用、计算过程和作图要求；仅把存在问题的题目送入修复。";
...
function renderTaskVisual(data) {
  ...
  if (currentPage === "result") {
    setResultPageState(status === "completed_with_issues" ? "issues" : status === "completed" ? "completed" : "loading", task);
  }
  if (status === "completed") {
    setVisual(
      "runVisualResult",
...
function startTaskPolling(taskId) {
  ...
  taskPollTimer = setInterval(async () => {
    if (document.hidden || currentPage !== "task" || activeTaskId !== taskId) return;
    const navigationVersion = taskNavigationVersion;
    try {
      const data = await api(`/api/tasks/${encodeURIComponent(taskId)}`);
      if (navigationVersion !== taskNavigationVersion || currentPage !== "task" || activeTaskId !== taskId) return;
      updateTaskSummary(data.task);
      renderTaskVisual(data);
      $("runResult").textContent = pretty(summarizeTaskStatus(data));
      maybeOpenActiveReviewDecision(data.task);
      if (data.task && ["completed", "completed_with_issues", "failed", "cancelled"].includes(data.task.status)) {
        ...
        await loadTasks();
        if (navigationVersion !== taskNavigationVersion || currentPage !== "task" || activeTaskId !== taskId) return;
        await taskFiles(taskId, navigationVersion);
...
async function taskFiles(taskId = ($("taskIdInput")?.value || activeTaskId || "").trim(), navigationVersion = taskNavigationVersion) {
  $("runResult").textContent = "读取中...";
  try {
    const data = await api(`/api/tasks/${encodeURIComponent(taskId)}/files`);
    if (navigationVersion !== taskNavigationVersion || activeTaskId !== taskId) return null;
...
function finalOutputFiles(files) {
  return finalOutputFileSpecs
    .filter((spec) => !(activeTaskAnalysisProfile === "question_only" && spec.label === "题目依据排查"))
    .map((spec) => {
...
function renderFiles(files) {
  ...
  if (!visibleFiles.length) {
    ...
    if (hint) {
      hint.className = "result-card muted-card";
      hint.innerHTML = activeTaskAnalysisProfile === "question_only"
...
function renderQuestionDetail(question) {
  ...
  const questionOnly = activeTaskAnalysisProfile === "question_only";
  const evidenceOnly = activeTaskAnalysisProfile === "textbook_evidence_only";
  const evidence = questionOnly ? null : resultBlock(question, "教材依据");
  ...
  const outlineItems = [
    ...
    ["result-question-answer", "答案", !evidenceOnly && !hideTopAnswer],
    ["result-question-knowledge", "知识点", true],
    ["result-question-evidence", "教材引用", !questionOnly],
    ["result-question-analysis", "解析", !evidenceOnly && !isTermExplanation && !isShortAnswerQuestion(question)],
    ["result-question-answer", "答案", !evidenceOnly && (isTermExplanation || isShortAnswerQuestion(question) || calculationAnswerAvailable)],
    ["result-question-options", "选项分析", !evidenceOnly && !isTermExplanation && Boolean(optionAnalysis?.text)],
    ["result-question-tips", "易错点", !evidenceOnly && !isTermExplanation && Boolean(tips?.text)],
    ["result-question-formulas", "公式", !evidenceOnly && !isTermExplanation && Boolean((question.formulas || []).length)],
  ...
  detail.innerHTML = `
    ...
    ${evidenceOnly || hideTopAnswer ? "" : `<section id="result-question-answer" class="answer-section">
      <h4>答案</h4>
    ...
    ${questionOnly ? "" : `<section id="result-question-evidence" class="evidence-section">
      <h4><i class="fas fa-book-open"></i>教材引用</h4>
      <p>${practiceMarkdown(evidence?.text || (question.evidence_ids || []).join("、") || "暂无教材引用")}</p>
    </section>`}
    ${evidenceOnly ? "" : isTermExplanation ? directAnswerHtml : `<section id="${isShortAnswerQuestion(question) ? "result-question-answer" : "result-question-analysis"}" class="analysis-section">
      <h4>${analysisTitle}</h4>
      <p>${practiceMarkdown(analysis?.text || "暂无解析内容")}</p>
    </section>`}
    ${!evidenceOnly && calculationAnswerAvailable ? `<section id="result-question-answer" class="analysis-section"><h4>答案</h4><p>${practiceMarkdown(solution.text)}</p></section>` : ""}
    ${!evidenceOnly && !isTermExplanation && optionAnalysis?.text ? `<section id="result-question-options" class="analysis-section"><h4>选项分析</h4><p>${practiceMarkdown(optionAnalysis.text)}</p></section>` : ""}
    ${!evidenceOnly && !isTermExplanation && tips?.text ? `<section id="result-question-tips" class="analysis-section"><h4>易错点及注意事项</h4><p>${practiceMarkdown(tips.text)}</p></section>` : ""}
    ${!evidenceOnly && !isTermExplanation && (question.formulas || []).length ? `<section id="result-question-formulas" class="formula-section"><h4>相关公式</h4>${(question.formulas || []).slice(0, 8).map((formula) => `<div><span class="practice-math">\\(${escapeHtml(formula.latex || "")}\\)</span><span>${escapeHtml(formula.source_note || "")}</span></div>`).join("")}</section>` : ""}
...
function renderTaskResultView(data) {
  ...
  if (subtitle) {
    const checkpointSummary = checkpointReusableCount + checkpointRedriveCount
      ...
      : "";
    subtitle.textContent = activeTaskAnalysisProfile === "textbook_evidence_only"
      ? `共定位 ${metrics.question_count || 0} 道题目`
      : `共解析 ${metrics.question_count || 0} 道题目${checkpointSummary}`;
  }
  renderResultQuestionList(questions);
  renderQuestionDetail(questions.find((q) => q.question_id === activeResultQuestionId));
}

async function loadTaskResultView(taskId = activeTaskId, navigationVersion = taskNavigationVersion) {
  const data = await api(`/api/tasks/${encodeURIComponent(taskId)}/result-view`);
  if (navigationVersion !== taskNavigationVersion || currentPage !== "result" || activeTaskId !== taskId) return null;
  activeTaskAnalysisProfile = data?.task?.analysis_profile || activeTaskAnalysisProfile;
...
async function hydrateResultPage() {
  ...
  if (!taskId) {
    setResultPageState("empty");
    syncResultFiles();
    return;
  }
  setResultPageState("loading");
  activeTaskId = taskId;
  $("taskIdInput").value = taskId;
  const navigationVersion = taskNavigationVersion;
  try {
    const data = await api(`/api/tasks/${encodeURIComponent(taskId)}`);
    if (navigationVersion !== taskNavigationVersion || currentPage !== "result" || activeTaskId !== taskId) return;
    if (data?.task?.is_generation_job) {
      activeTaskId = "";
      $("taskIdInput").value = "";
      setResultPageState("empty");
      return;
    ...
    $("runResult").textContent = pretty(summarizeTaskStatus(data));
    await loadTaskResultView(taskId, navigationVersion);
    await taskFiles(taskId, navigationVersion);
    await loadReview(taskId, navigationVersion);
  } catch (error) {
    if (navigationVersion !== taskNavigationVersion || currentPage !== "result" || activeTaskId !== taskId) return;
    activeTaskId = "";
    $("taskIdInput").value = "";
...
function renderReviewRows(rows) {
  ...
  for (const [rowIndex, row] of (rows || []).entries()) {
    ...
    const notes = (row.notes || []).map((x) => `<li>${escapeHtml(publicDiagnosticMessage(x, row.question_id))}</li>`).join("");
    const questionOnly = activeTaskAnalysisProfile === "question_only";
    ...
    body.innerHTML = `
      <div class="review-map-grid">
        ...
        ${questionOnly ? "" : `<p><strong>教材依据状态</strong><span>${escapeHtml(evidenceStatus)}</span></p>`}
      </div>
      <details class="review-technical-details"><summary>技术信息</summary><p><strong>内部题目标识</strong><span>${escapeHtml(row.question_id || "-")}</span></p></details>
...
async function loadReview(taskId = ($("taskIdInput")?.value || activeTaskId || "").trim(), navigationVersion = taskNavigationVersion) {
  const button = $("loadReviewBtn");
  const originalLabel = button?.textContent || "读取审查报告映射";
  if (button) {
    button.disabled = true;
  ...
  try {
    ...
    if (navigationVersion !== taskNavigationVersion || activeTaskId !== taskId) return null;
    activeTaskAnalysisProfile = data.analysis_profile || activeTaskAnalysisProfile;
    ...
    if (button) button.textContent = "已更新审查映射";
  } catch (err) {
    if (navigationVersion !== taskNavigationVersion || activeTaskId !== taskId) return null;
    $("reviewResult").textContent = String(err);
    $("reviewList").innerHTML = `<div class="review-empty">审查报告映射读取失败：${escapeHtml(String(err).replace(/^Error:\s*/, ""))}</div>`;
  } finally {
    if (button) {
      button.removeAttribute("aria-busy");
      setTimeout(() => {
...
$("testExamModelRoutesBtn")?.addEventListener("click", () => testExamModelRoutes().catch((error) => {
  setText("examModelTestHint", providerErrorAdvice(error).body);
}));
...
$("taskSearchInput")?.addEventListener("input", (event) => {
  activeTaskQuery = event.target.value || "";
  taskManagerPage = 1;
  $("taskSearchClearBtn")?.classList.toggle("hidden", !activeTaskQuery.trim());
  updateTaskActiveFilterSummary();
  renderTaskManager();
});
...
$("taskManagerEmptyAction")?.addEventListener("click", () => {
  const action = $("taskManagerEmptyAction");
  if (action?.dataset.emptyAction === "create") {
    const kind = action.dataset.emptyKind;
    if (kind === "format") openWordFormatReviewer();
    else if (kind === "knowledge") goToPage("knowledge-models");
    else if (kind === "practice") goToPage("practice-models");
    else startWizard();
  } else if (action?.dataset.emptyAction === "home" || latestTasks.length === 0) goToPage("home");
  else clearTaskManagerFilters();
});
...
$("practiceDownloadAllBtn")?.addEventListener("click", () => {
  const data = exportablePracticeSet();
  if (!data) return;
  prepareOrDownloadPracticeWord(data, $("practiceDownloadAllBtn"), practiceWordFilename(data, `全部${data.exercises.length}题`)).catch((error) => {
    platformAlert(String(error).replace(/^Error:\s*/, ""), { title: "题目 Word 操作未完成", tone: "danger" });
  });
});
